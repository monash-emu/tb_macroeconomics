...
def get_latent_log_likelihood(
    ...
    The corresponding modelled quantity is everyone previously 
...
def get_notification_log_likelihood(results: dict):
    ...
    (Notifications through active case finding are not included
    in this target, although this does not affect calibration 
    because the simulated ACF programs have not started by the time
    the model is compared against notification counts.)
...
def get_pulm_prev_log_likelihood(
    ...
    (PLOS One). This study reports prevalent cases as a proportion of 
    the adult population. Being a proportion, this quantity was compared 
    on the log-odds scale with a standard deviation of {{PROP_LOGIT_SD}}.
    """
...
def get_mixing_log_likelihood(
    params: dict,
    age_weights: jnp.ndarray,
    weight_ends: jnp.ndarray,
    group_popsize: jnp.ndarray,
    pop_ends: jnp.ndarray,
    fert_padded: jnp.ndarray,
    fert_ends: jnp.ndarray,
    conmat: jnp.ndarray,
):
    r"""Get the likelihood contribution from the age-mixing matrix.

    Args:
        params: Parameter set containing the mixing scalars
        age_weights: Within-group single-age weights by year
        weight_ends: Start and finish of the weight index
        group_popsize: Population sizes by age group and year
        pop_ends: Start and finish of the population index
        fert_padded: Fertility data padded with zeroes
        fert_ends: Start and finish of the fertility index
        conmat: Spectral-radius-normalised synthetic contact matrix
    ...
    `conmat` package in R. Both matrices are normalised by their spectral
    radius before comparison, so that only the relative strength of
    age-to-age mixing is targeted.

    The discrepancy between the matrices is summarised by the Canberra distance, 
    which gives comparable weight to low- and high-contact cells.
    This distance is treated as a residual around zero,
    $d \sim \mathcal{N}(0, \sigma_{m})$, where $\sigma_{m}$ is the
    "{{mixing_dist_sd}}". Unlike the fixed error scales used for the
    ...
    distance = canberra_distance(modelled, conmat)
    return dist.Normal(0.0, params["mixing_dist_sd"]).log_prob(distance)
...
def make_log_likelihood(
    ...
    mixing target to adjust to the data, while ensuring that its range
    remains comparable to the epidemiological contributions.
    ...
    pop_arr = jnp.array(group_popsize)
    pop_ends = jnp.array(group_popsize.index[[0, -1]])
    fert_arr = jnp.array(fert_padded)
    fert_ends = jnp.array(fert_padded.index[[0, -1]])
    conmat = get_norm_conmat(ISO3)
